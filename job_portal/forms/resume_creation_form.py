...
class SkillForm(forms.ModelForm):
    class Meta:
        model = Skill
        fields = ['skill_name', 'skill_category_choice', 'skill_category_other', 'proficiency_level']
        widgets = {
            'skill_name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Skill Name (e.g., Python)'}),
            'skill_category_choice': forms.Select(attrs={'class': tailwind_select_classes}),
            'skill_category_other': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Custom Category (if Other is selected)'}),
            'proficiency_level': forms.NumberInput(attrs={'class': tailwind_input_classes, 'min': 0, 'max': 100, 'step': 5}),
        }
...
class ExperienceForm(forms.ModelForm):
    class Meta:
        model = Experience
        fields = ['job_title', 'employer', 'location', 'start_date', 'end_date', 'is_current']
        widgets = {
            'job_title': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Job Title'}),
            'employer': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Company Name'}),
            'location': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'City, State/Country or Remote'}),
...
class ExperienceBulletPointForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['description']
        widgets = {
            'description': forms.Textarea(attrs={'class': tailwind_textarea_classes, 'rows': 2, 'placeholder': 'Key achievement or responsibility (e.g., Led a team of 5 engineers...)'}),
        }
...
class EducationForm(forms.ModelForm):
    class Meta:
        model = Education
        fields = ['school_name', 'location', 'degree_name', 'degree_type',
                  'field_of_study', 'graduation_date', 'gpa', 'description']
        widgets = {
            'school_name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Institution Name'}),
            'location': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'City, State/Country'}),
            'degree_name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Degree Title (e.g., B.S. in Computer Science)'}),
            'degree_type': forms.Select(attrs={'class': tailwind_select_classes}),
            'field_of_study': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Field of Study (e.g., Computer Science)'}),
            'graduation_date': forms.DateInput(attrs={'class': tailwind_date_classes, 'type': 'date'}),
            'gpa': forms.NumberInput(attrs={'class': tailwind_input_classes, 'placeholder': 'GPA (e.g., 3.8)', 'step': '0.01'}),
...
class ProjectForm(forms.ModelForm):
    class Meta:
        model = Project
        fields = ['project_name', 'summary', 'start_date', 'completion_date', 'project_link', 'github_link']
        widgets = {
            'project_name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Project Title'}),
            'summary': forms.Textarea(attrs={'class': tailwind_textarea_classes, 'rows': 4, 'placeholder': 'Describe the project, your role, and technologies used.'}),
            'start_date': forms.DateInput(attrs={'class': tailwind_date_classes, 'type': 'date'}),
            'completion_date': forms.DateInput(attrs={'class': tailwind_date_classes, 'type': 'date'}),
            'project_link': forms.URLInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Link to live project'}),
            'github_link': forms.URLInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Link to GitHub repository'}),
        }
...
class ProjectBulletPointForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['description']
        widgets = {
            'description': forms.Textarea(attrs={'class': tailwind_textarea_classes, 'rows': 2, 'placeholder': 'Key feature or contribution (e.g., Implemented user authentication...)'}),
        }
...
class CertificationForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['name', 'issuing_organization', 'issue_date', 'expiration_date',
                 'credential_id', 'score', 'link', 'description']
        widgets = {
            'name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Certification Name'}),
            'issuing_organization': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Organization Name'}),
            'issue_date': forms.DateInput(attrs={'class': tailwind_date_classes, 'type': 'date'}),
            'expiration_date': forms.DateInput(attrs={'class': tailwind_date_classes, 'type': 'date'}),
            'credential_id': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Credential ID'}),
            'score': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Score/Grade (Optional)'}),
            'link': forms.URLInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Verification URL (Optional)'}),
            'description': forms.Textarea(attrs={'class': tailwind_textarea_classes, 'rows': 2, 'placeholder': 'Additional details about the certification'}),
        }
...
class LanguageForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'language_name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Language (e.g., English, Spanish)'}),
...
class CustomDataForm(forms.ModelForm):
    class Meta:
        model = CustomData
        fields = ['name', 'institution_name', 'completion_date', 'bullet_points', 'description', 'link']
        widgets = {
            'name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Section Title (e.g., Awards and Honors)'}),
            'institution_name': forms.TextInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Organization/Institution (if applicable)'}),
            'completion_date': forms.DateInput(attrs={'class': tailwind_date_classes, 'type': 'date'}),
            'bullet_points': forms.Textarea(attrs={'class': tailwind_textarea_classes, 'rows': 4, 'placeholder': 'Add bullet points, one per line'}),
            'description': forms.Textarea(attrs={'class': tailwind_textarea_classes, 'rows': 3, 'placeholder': 'Additional description for this section'}),
            'link': forms.URLInput(attrs={'class': tailwind_input_classes, 'placeholder': 'Relevant URL (if applicable)'}),
        }
...
ExperienceBulletPointInlineFormSet = inlineformset_factory(
    ...
    fields=['description'], extra=1, can_delete=True
)
...
SkillInlineFormSet = inlineformset_factory(
    ...
    extra=3, can_delete=True, fk_name='resume'
)
